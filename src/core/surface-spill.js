...
function simulateSurfaceSpill({ rainfallMmHr, imperviousPct, catchmentAreaHa, elevationM, drainageRisk, swmm, topologyConfidence = 0, calibration } = {}) {
  ...
  const drainRemovalMmHr = clamp(32 * (1 - risk * .68) * (.72 + topologyConfidence * .28), 4, 34);
  const runoffMmHr = Math.max(0, rain * impervious - drainRemovalMmHr);
  const excessVolumeM3 = runoffMmHr / 1000 * areaHa * 10000;
  const roadStorageAreaM2 = clamp(areaHa * 10000 * (.035 + (elevationM != null && elevationM < 8 ? .015 : 0)), 180, 1800);
  const centralDepthM = clamp(excessVolumeM3 / roadStorageAreaM2 + knownFlooding, 0, 1.5);
  ...
  return {
    tier: 'surface-spill-balance', rainMmHr: rain, runoffMmHr, drainRemovalMmHr,
