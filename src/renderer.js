...
async function runScenario(rainfallMmHr) {
  ...
  try {
    ...
    const scenario = result.scenario; const depthCm = scenario.projectedRoadWaterM == null ? null : Math.round(scenario.projectedRoadWaterM * 100);
    const water = document.querySelector('#streetWater');
    water.style.height = depthCm == null ? '0' : `${Math.min(77, 8 + scenario.projectedRoadWaterM * 78)}%`;
    document.querySelector('#streetScene').classList.toggle('has-water', depthCm > 0);
    document.querySelector('#streetEmpty').textContent = depthCm == null ? 'Depth is intentionally withheld: this street is outside the calibrated pilot.' : depthCm ? '' : 'This rainfall scenario does not project standing road water at this point.';
    title.textContent = depthCm == null ? 'Road-water depth is not validated here' : depthCm ? `About ${depthCm} cm of road water (±${Math.round(scenario.uncertaintyM * 100)} cm)` : 'No standing road water projected';
    const drain = scenario.selectedDrain;
    detail.textContent = `${rainfallMmHr} mm/h at ${scenarioPoint.label}. Snapped to ${drain?.label || 'the nearest mapped drain'}${drain?.snappedDistanceM != null ? ` (${Math.round(drain.snappedDistanceM)} m away)` : ''}; its measured slope is ${drain?.slope ? (drain.slope * 100).toFixed(2) : 'unknown'}%. ${scenario.disclaimer}`;
    ...
    const networkText = `${scenario.modelScope}; ${scenario.localNetwork?.length || 0} inferred nearby drain connection(s); built-form runoff proxy ${Math.round(scenario.runoff?.imperviousPct || 0)}% impervious.`;
